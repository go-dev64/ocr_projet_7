...
def clean_csv(data, budget=500):
    """
    clean csv file -> remove elements which the value of cost and
    profit is lower than zero.
    create new column(index 3) ->
    gain =column[price] * column[profit] /100
    crete new column(index 4)->
    round price = round(data["price"], 2) *10
    :param data: csv file
...
def filter_data(data, budget, element_list):
    """
    returns the data for which the value of the column index 1
    (cost_action) is lower than the budget
    :param data:np.array
    :param budget: number -> remaining budget =
        initial budget - total cost of element_list
...
def get_best_invest(elements_list, budget=5000):
    ...
    for i in np.arange(1, len(elements_list) + 1):
        ...
        for w in np.arange(1, budget + 1):
            ...
            if elements_list[i - 1][4] <= w:
                # On détermine le profit de l'élément actuel
                # + celle de l'élément au cout précédent.
                # On garde le max
                # et l'affecte à la position de la matrice pour i et w.
                matrice[i][w] = max(
                    elements_list[i - 1][3]
                ...
                # Sinon valeur pour la position actuelle=matrice[i-1][w]
    ...
    while w >= 0 and n >= 0:
        ...
        # Si la valeur de la matrice[n][w] =
...
def print_result_dynamique(actions_list):
    ...
    if len(actions_list_filtered) > 0:
        list_action_new_budget = get_best_invest(
